...
namespace EazyOnRent.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("SignUp")]
        public async Task<dynamic> SignUp(User user)
        {
            dynamic result = new ExpandoObject();
            try
            {

                if (string.IsNullOrEmpty(user.Mobile) || user.Mobile.Length != 10)
                {
                    result.ResponseCode = "999";
                    result.ResponseMessage = "Please enter valid mobile number.";
                    return result;
                }
                ...
                {
                    var listerExist = await _context.Listers.Where(x => x.Mobile == user.Mobile).FirstOrDefaultAsync();
                    if (listerExist != null)
                    {
                        result.ResponseCode = "999";
                        result.ResponseMessage = "Lister mobile number already registered.";
                        return result;
                    }

                    Lister lister = new Lister();
                    lister.Name = user.Name;
                    lister.Email = user.Email;
                    lister.Mobile = user.Mobile;
                    lister.Password = user.Password;
                    lister.Lat = user.Lat;
                    lister.Long = user.Long;
                    lister.LatLongAddress = user.LatLongAddress;
                    lister.CreatedDate = DateTime.Now.Date;
                    lister.Status = true;
                    lister.UpdatedOn = user.UpdatedOn;
                    await _context.Listers.AddAsync(lister);
                ...
                {
                    var renterExist = await _context.Renters.Where(x => x.Mobile == user.Mobile).FirstOrDefaultAsync();
                    if (renterExist != null)
                    {
                        result.ResponseCode = "999";
                        result.ResponseMessage = "Lister mobile number already registered.";
                    ...
                    lister.CreatedDate = DateTime.Now.Date;
                    lister.Status = true;
                    await _context.Renters.AddAsync(lister);
                    var results = await _context.SaveChangesAsync();
                    result.ResponseCode = "000";
                    result.ResponseMessage = "User registered successfully.";
                    return result;
